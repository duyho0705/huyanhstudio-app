    ...
    <>
      <header className="sticky top-0 z-40 bg-white/70 backdrop-blur-2xl border-b border-slate-100 h-20 flex items-center justify-between px-6 md:px-10">
        <div className="flex items-center gap-6">
          <button
            className="w-10 h-10 flex items-center justify-center bg-slate-50 hover:bg-slate-100 rounded-none text-slate-600 transition-all active:scale-90 lg:hidden"
            onClick={toggleSidebar}
          >
            <Menu size={20} />
          </button>

          <div className="hidden md:flex items-center gap-3 px-5 py-2.5 bg-slate-50/50 rounded-none border border-slate-100 group transition-all focus-within:bg-white focus-within:ring-4 focus-within:ring-slate-100 focus-within:border-slate-200">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <div className="flex items-center gap-4 p-1 rounded-none hover:bg-slate-50 group cursor-pointer transition-all border border-transparent hover:border-slate-100">
              <div className="relative">
                <div className="w-11 h-11 bg-slate-900 rounded-none flex items-center justify-center text-white shadow-xl shadow-slate-200 transition-transform group-hover:scale-95">
                  <User size={20} />
                </div>
      ...
      {/* Notifications */}
      {notification.show && (
        <div className="fixed top-10 right-10 z-[2000] px-8 py-5 bg-white rounded-none shadow-2xl border border-slate-100 flex items-center gap-5 animate-in slide-in-from-right duration-500">
          <div className={`p-3 rounded-none ${notification.type === "success" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
      ...
      >
        <div className="p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <button onClick={() => setBookingsModal(false)} className="w-10 h-10 bg-slate-50 rounded-none flex items-center justify-center text-slate-400 hover:text-slate-900 transition-all">
              <Home size={18} />
            </button>
          </div>
          <AdminBookings />
