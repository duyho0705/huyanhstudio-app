    ...
    <div className="flex flex-col xl:flex-row items-stretch xl:items-center gap-2 xl:gap-3 w-full">
      ...
      <div className="flex flex-col sm:flex-row items-stretch gap-2 flex-1">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto sm:flex-1">
          <Select
            placeholder={<span className="font-medium text-slate-500 text-[13px]">{t('admin.bookings.status_placeholder')}</span>}
            ...
            {bookingStatuses.map((s) => (
              <Option key={s.value} value={s.value}>
                <div className="flex items-center gap-2 font-medium text-[13px] text-slate-700">
                  ...
                  <span className="truncate">{t(`user.booking.statuses.${s.value}`)}</span>
                </div>
              </Option>
            ))}
          </Select>
          ...
          >
            <span className="hidden sm:inline">{t('admin.bookings.reset_filters')}</span>
          </Button>
          ...
          {selectedRowKeys.length > 0 && (
            <Button
              ...
              icon={<Trash2 size={15} />}
            >
              <span className="hidden sm:inline font-medium">{t('admin.bookings.delete')}</span> ({selectedRowKeys.length})
            </Button>
          )}
