  ...
  const columns = [
    {
      ...
      render: (name) => (
          <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-slate-900 flex items-center justify-center text-white shrink-0 shadow-lg shadow-slate-200">
                  <Home size={20} />
              </div>
              <div>
    ...
    },
    {
      ...
      width: 180,
      render: (rate) => (
        <div className="px-4 py-2 bg-green-50 text-green-700 rounded-2xl border border-green-100 inline-flex items-center gap-2">
            <DollarSign size={14} />
      ...
      render: (_, record) => (
        <div className="flex items-center justify-center gap-2">
          <Button 
            ...
            className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center justify-center p-0"
            icon={<Edit size={18} />}
          />
          <Button 
            type="text" 
            ...
            className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-600 transition-all flex items-center justify-center p-0"
            icon={<Trash2 size={18} />}
          />
        </div>
      ),
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center justify-between">
        ...
        <Button
          ...
          className="h-14 px-8 bg-slate-900 hover:bg-slate-800 rounded-2xl border-none font-black uppercase tracking-widest text-xs shadow-xl shadow-slate-200 flex items-center gap-3 transition-all active:scale-95"
        >
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden bg-white">
        <Table
          columns={columns}
          dataSource={studios}
          rowKey="id"
        ...
        {pagination.total > 0 && (
          <div className="p-8 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="px-6 py-3 bg-slate-50 rounded-2xl text-[11px] font-black text-slate-500 uppercase tracking-widest">
            ...
            <div className="flex items-center gap-2">
                <Button
                    className="h-12 w-12 flex items-center justify-center rounded-2xl border-slate-200 text-slate-600 hover:text-blue-600 hover:border-blue-600 transition-all font-black shadow-sm bg-white"
            ...
            {selectedStudio && (
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 mb-8 space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <Tag className="!m-0 bg-white text-slate-600 font-bold border-slate-200 rounded-lg">{selectedStudio.capacity} người</Tag>
                    </div>
                </div>
            )}

            <div className="flex gap-4">
                <Button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 h-12 rounded-2xl font-black uppercase tracking-widest text-[10px] border-slate-200">Hủy</Button>
                <Button onClick={confirmDelete} danger type="primary" className="flex-1 h-12 rounded-2xl bg-red-600 border-none font-black uppercase tracking-widest text-[10px] shadow-lg shadow-red-100 transition-all hover:bg-red-700">Xác nhận xóa</Button>
            </div>
        </div>
      </Modal>
    </div>
